...
# UTC 16:00 on 1 Aug is 04:00 NZST on 2 Aug — service date must win over folder date.
ymd=2026/08/01
hh=16
...
test "${late_routes}" = "1"
late_day="$(duckdb -csv -c "SELECT DISTINCT CAST(day AS VARCHAR) FROM read_parquet('${ARCHIVE_ROOT}/derived/late-trips/2026-08.parquet');" | tail -n 1)"
...
test "${tp_rows}" -ge 1
tp_t1_day="$(duckdb -csv -c "SELECT CAST(day AS VARCHAR) FROM read_parquet('${ARCHIVE_ROOT}/derived/trip-performance/2026-08.parquet') WHERE trip_id = 't1' AND observed;" | tail -n 1)"
test "${tp_t1_day}" = "2026-08-02"
tp_t1="$(duckdb -csv -c "SELECT scheduled, observed FROM read_parquet('${ARCHIVE_ROOT}/derived/trip-performance/2026-08.parquet') WHERE trip_id = 't1' AND observed;" | tail -n 1)"
test "${tp_t1}" = "true,true"
# Latest STU delay wins on a tied feed_timestamp; later CANCELED with no STUs still counts.
tp_t1_census="$(duckdb -csv -c "SELECT cancelled, delay_seconds FROM read_parquet('${ARCHIVE_ROOT}/derived/trip-performance/2026-08.parquet') WHERE trip_id = 't1' AND observed;" | tail -n 1)"
...
test "${sd_t1}" -eq 2
sd_day="$(duckdb -csv -c "SELECT DISTINCT CAST(day AS VARCHAR) FROM read_parquet('${ARCHIVE_ROOT}/derived/stop-delay/2026-08.parquet');" | tail -n 1)"
test "${sd_day}" = "2026-08-02"
